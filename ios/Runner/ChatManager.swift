...
class ChatManager: NSObject {
    ...
    func updateStorage() {
        ...
        if QBChat.instance.isConnected == false {
            self.delegate?.chatManager(self, didFailUpdateStorage: "SA_STR_NETWORK_ERROR".localizedUppercase)
            return
        }
...
//        })
        ...
        loadGroup.notify(queue: DispatchQueue.main) {
            if message.isEmpty {
                self.delegate?.chatManager(self, didUpdateStorage: "SA_STR_COMPLETED".localizedUppercase)
            } else {
...
//            }
    ...
    func createGroupDialog(withName name: String,
        ...
        QBRequest.createDialog(chatDialog, successBlock: { response, dialog in
            dialog.join(completionBlock: { (error) in
                ...
                self.delegate?.chatManager(self, didUpdateStorage: "SA_STR_CREATE_NEW".localizedUppercase + dialogName)
    ...
    private func errorMessage(response: QBResponse) -> String? {
        ...
        if response.status.rawValue == 502 {
            errorMessage = "SA_STR_BAD_GATEWAY".localizedUppercase
        } else if response.status.rawValue == 0 {
            errorMessage = "SA_STR_NETWORK_ERROR".localizedUppercase
        } else {
